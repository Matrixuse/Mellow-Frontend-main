...
const AdminLoginForm = ({ onLogin }) => {
    ...
    return (
        <form onSubmit={handleAdminLogin} className="space-y-4">
            ...
            <div><label className="text-sm font-medium text-gray-300">Username</label><input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full px-3 py-2 mt-1 text-white bg-[#282828] border border-gray-600 rounded-md" required /></div>
            <div><label className="text-sm font-medium text-gray-300">Email ID</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-3 py-2 mt-1 text-white bg-[#282828] border border-gray-600 rounded-md" required /></div>
            <div><label className="text-sm font-medium text-gray-300">Password</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-2 mt-1 text-white bg-[#282828] border border-gray-600 rounded-md" required /></div>
            {error && <p className="text-sm text-red-400 text-center">{error}</p>}
            <button type="submit" disabled={isLoading} className="w-full py-2 font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:bg-[#5f5f5f]">{isLoading ? 'Authenticating...' : 'Login as Admin'}</button>
        </form>
    );
};
...
const UploadSongForm = ({ onSongUploaded }) => {
    ...
    return (
        <form onSubmit={handleUpload} className="space-y-4">
            ...
            {/* Naye Text Inputs */}
            <div><label className="text-sm font-medium text-gray-300">Title</label><input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-3 py-2 mt-1 text-white bg-[#282828] border border-gray-600 rounded-md" placeholder="Enter song title" required /></div>
            <div><label className="text-sm font-medium text-gray-300">Artist</label><input type="text" value={artist} onChange={(e) => setArtist(e.target.value)} className="w-full px-3 py-2 mt-1 text-white bg-[#282828] border border-gray-600 rounded-md" placeholder="Enter artist names (comma-separated)" required /></div>
                        ...
                        <button
                            ...
                            className={`px-3 py-2 text-sm rounded-md transition-colors ${
                                selectedMoods.includes(mood)
                                    ...
                                    : 'bg-[#282828] text-gray-300 hover:bg-[#3f3f3f]'
                            }`}
                        >
                            {mood}
                        </button>
            ...
            <button type="submit" disabled={isLoading} className="w-full py-2 font-semibold text-white bg-green-600 rounded-md hover:bg-green-700 disabled:bg-[#5f5f5f] flex items-center justify-center gap-2">
...
const AdminPanel = ({ onClose, onSongUploaded }) => {
    ...
    return (
        <div
            className="relative w-full max-w-lg mx-4 md:mx-0 p-6 space-y-4 bg-[#1f1f1f] rounded-lg shadow-2xl border border-gray-700 max-h-[75vh] md:max-h-[80vh] overflow-y-auto"
        >
