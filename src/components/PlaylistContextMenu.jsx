...
const PlaylistContextMenu = ({ playlist, onAddToFavorites = () => {}, onSavePlaylist = () => {} }) => {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 bg-[#1f1f1f] border border-gray-700 rounded-md shadow-lg z-50 w-44">
            <button
              onClick={(e) => { e.stopPropagation(); handleAddToFavorites(); }}
              className="w-full text-left px-3 py-2 text-sm text-gray-200 hover:bg-[#282828] flex items-center gap-2"
            >
              <Heart size={14} />
              Add to Favorites
            </button>
          <button
            ...
            className="w-full text-left px-3 py-2 text-sm text-gray-200 hover:bg-[#282828] flex items-center gap-2 border-t border-gray-700"
          >
            <Bookmark size={14} />
            Save Playlist
          </button>
